...
const vcard = 'BEGIN:VCARD\n' //JANGAN DI GANTI
            + 'VERSION:3.0\n' //JANGAN DI GANTI
            + 'FN:Owner KhuuzaifiGans\n' //GANTI NAMA LUH
            + 'ORG: INVINCIBLTY - BOT;\n' //GANTI NAMA BOT LUH
            + 'TEL;type=CELL;type=VOICE;waid=19128254776:+1 (912) 825-4776\n' // GANTI NOMOR LUH,TAPI JANGAN DIUBAH POLANYA
...
ator = 'FollowIG'
namo = 'khuzaifigans_*'
cr = '*FollowIG:khuzaifigans_🍁*'//TERSERAH MAU GANTI ATAU KAGAK
/*************************************/

/******** OWNER NUMBER**********/
const ownerNumber = ["19128254776@s.whatsapp.net"] //GANTI NOMOR LUH
/************************************/
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
		...
		if (!isRegistered) return reply(ind.noregis())
			if (args.length < 1) return reply(`Contoh: ${prefix}joox Kokoronashi`)
                    ...
                    get_result = get_result.result
		    get_info = get_result.info
		    txt = '*_[ FITUR UP BY KhuzaifiGans ]_*\n\n'
